...
std::vector<std::vector<double> > get_interocular_distances_194 (
    const std::vector<std::vector<full_object_detection> >& objects
);
/*!
    ensures
...
{  
    try
    {
        ...
        std::string trainer_oversampling_amount = "20";
        std::string trainer_nu = "1.0";
        ...
        trainer.set_oversampling_amount( std::stoi( trainer_oversampling_amount ) );
        // I'm also reducing the capacity of the model by explicitly increasing
        // the regularization (making nu smaller) and by using trees with
        // smaller depths.
        trainer.set_oversampling_amount( std::stoi( trainer_oversampling_amount ) );
        trainer.set_nu( std::stof( trainer_nu ) );
...
double interocular_distance_194 (
    const full_object_detection& det
)
{
    dlib::vector<double,2> l, r;
    ...
    for (unsigned long i = 33; i <= 37; ++i)
    {
        r += det.part(i);
        ++cnt;
    }
    for (unsigned long i = 38; i <= 42; ++i)
    {
        r += det.part(i);
        ++cnt;
    }
...
)
{
    ...
    for (unsigned long i = 0; i < objects.size(); ++i)
    {
        ...
        {
            if (objects[i][j].num_parts() == 68)
                temp[i].push_back(interocular_distance(objects[i][j]));
            if (objects[i][j].num_parts() == 194)
                temp[i].push_back(interocular_distance_194(objects[i][j]));
        }
    }
    return temp;
}
