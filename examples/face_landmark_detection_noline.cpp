        ...
        for (int i = 2; i < argc; ++i)
        {
            ...
            for (unsigned long j = 0; j < dets.size(); j++)
            {
                ...
                for (unsigned long i = 0; i < per_det_shapes; i++)
                {
                    ...
                    rgb_pixel color = rgb_pixel(255/shapes.size()*dets.size()*i,128,128);
                    for (unsigned long idx = 0; idx < shape_temp.num_parts(); idx++)
                    {
                        image_display::overlay_circle circle(shape_temp.part(idx), 5, color);
