...
// ----------------------------------------------------------------------------------------
                ...
                for (unsigned long i = 0; i < get_num_trees_per_cascade_level(); ++i)
                {
                    if (_method == train_method::decision_tree)
                        forests[cascade].push_back(
                            make_regression_tree(tp, samples, pixel_coordinates[cascade]));

                    // chin add fern
                    if (_method == train_method::random_fern)
                        forests[cascade].push_back(
                            make_regression_fern(tp, samples, pixel_coordinates[cascade]));
    ...
    private:
        ...
        ) const
        {
            ...
            generate_features(tp, samples, feats);
        ...
        ) const
        {
            ...
            for (unsigned long i = 0; i < samples.size(); ++i)
            {
                ...
                yproj = sum( pointwise_multiply(
                        matrix_cast<double>(samples[i].diff_shape), 
                        proj)
                    );
